...
from datasets.mnist import parse_function
from Glow import Glow
import tensorflow_probability.python.bijectors as tfb
tfd = tfp.distributions


class _Glow(tf.keras.Model):
    def __init__(self):
        super(_Glow, self).__init__()
        glow = Glow([None, 32, 32, 1], levels=2)
...
def train_step(model, optimizer, x):
    with tf.GradientTape() as tape:
        code = model(x)
        loss = -tf.reduce_mean(model.flow.log_prob(code))
...
def main():
    trainset = tf.data.TFRecordDataset(
        os.path.join(
            'mnists',
            'trainset.tfrecord')).map(parse_function).shuffle(10000).batch(50)
    ...
    while True:
        ...
        for (images, _) in trainset:
            ...
            if tf.equal(optimizer.iterations % 100, 0):
                current_loss = avg_loss.result()
                print('step {} loss {:.6f}'.format(i, current_loss))
                if current_loss < 0.8:
                    break
            # avg_loss.reset()
        else:
...
#     # Create a model
#     base_dist = tfp.distributions.Normal(loc=0., scale=1.)
#     glow = tfb.Invert(Glow([None, 32, 32, 1], levels=3))
...
#         os.path.join(
#             'mnists',
#             'trainset.tfrecord')).map(parse_function).shuffle(10000).batch(100)

#     base_dist = tfp.distributions.Normal(loc=0., scale=1.)
#     glow = tfb.Invert(Glow([None, 32, 32, 1], levels=2))
